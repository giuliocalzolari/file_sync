...
class ChangeHandler(FileSystemEventHandler):
	...
	def push_to_s3(self, sourcepath):
	  try:
		...
		if self.current_repl["replace"] != "":
			destpath = sourcepath.replace(self.current_repl["replace"][0],self.current_repl["replace"][1])
		else:
			destpath = sourcepath

		# connect to the bucket
		conn = boto.connect_s3(self.current_repl["acces_key"],self.current_repl["secret_key"])
	...
	def delete_s3(self, file_name_dest):
	  try:
		...
		if self.current_repl["replace"] != "":
			file_name = file_name_dest.replace(self.current_repl["replace"][0],self.current_repl["replace"][1])
		else:
			file_name = file_name_dest

		self.log.debug("delete: "+file_name+" from s3 bucket:"+self.current_repl["bucket"])
	...
	def push_to_sftp(self, sourcepath):
		...
		try:
			...
			ssh.connect('myserver.compute-1.amazonaws.com', username=self.current_repl["username"], pkey=mykey)
			sftp = ssh.open_sftp()
	...
	def on_any_event(self, event):
		...
		if event.is_directory:
			self.log.debug("DIR : "+event.src_path +" "+event.event_type + " ")
		else:
			self.log.debug("FILE: "+event.src_path +" "+event.event_type + " ")
					...
					for repl in self.config["replicator"]:
						...
						if c_url.startswith( 'ec2://' ):
							...
							if (t1-t0) >= self.config["replicator"][repl]["refresh"] or start:
								self.log.debug("Execute ec2_update_discovery for: "+repl)
								self.ec2_auto[c_url.replace('ec2://', "")] = self.ec2_update_discovery(self.config["replicator"][repl])
								t0 = t1 
								start = False
								# print self.ec2_auto
...
if __name__ == "__main__":
	...
	fs.add_param("-d", "--detail", help="What to search for: ['datastore','vms','hostsystems','dvport','templates','resourcepool','datacentres','folders','vlans']", default=None, required=False, action="store")
	fs.add_param("-e", "--extra", help="Extra detail to add to a given mode", default=None, required=False, action="store")
	fs.add_param("-c", "--config", help="Change the Configuration file to use", default="./config.yaml", required=False, action="store")
	fs.run()
